...
from config.user import db_host, db_user, db_pass, db_database
...
class MySQL:
    ...
    def insertOrUpdate(self, **kwargs):
        isExist = self.select(table=kwargs['table'], **kwargs['params']['columns'],
                              condition=kwargs['params']['condition'])
        if isExist:
            self.update(table=kwargs['table'], params=kwargs['params'])
        else:
            self.insert(table=kwargs['table'], columns=list(kwargs['params']['columns'].keys()),
                        values=list(kwargs['params']['columns'].values()))

    def connect(self):
        self.connection = connector.connect(host=self.host, user=self.user, password=self.password, database=self.db)
